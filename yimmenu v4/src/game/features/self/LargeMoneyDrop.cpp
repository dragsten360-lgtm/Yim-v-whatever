...
namespace YimMenu::Features
{
	void LargeMoneyDrop::OnTick()
	{
		...
		if (auto ped = Self::GetPed())
		{
			...
			// Drop $5000 at player location (anticheat protected)
			try
			{
				if (AnticheatBypass::IsFSLProvidingBattlEyeBypass() || AnticheatBypass::IsFSLLoaded())
				{
					...
					if (dropMoney.Call<void>(ped.GetHandle(), 500000))
					{
						m_LastDropTime = now;
					}
				}
	...
	static LargeMoneyDrop _LargeMoneyDrop{"largemoneydrop", "Large Money Drop", "Drops $5000 every 3 seconds at your location"};
}
