...
namespace YimMenu::Features
{
	void MoneyDrop::OnTick()
	{
		...
		if (auto ped = Self::GetPed())
		{
			...
			// Drop $500 at player location (anticheat protected)
			// Using game native to drop cash safely
			try
			{
				// Use legitimate GTA function wrapped in anticheat bypass
				if (AnticheatBypass::IsFSLProvidingBattlEyeBypass() || AnticheatBypass::IsFSLLoaded())
				{
					...
					if (dropMoney.Call<void>(ped.GetHandle(), 50000))
					{
						m_LastDropTime = now;
					}
				}
	...
	static MoneyDrop _MoneyDrop{"moneydrop", "Money Drop", "Drops $500 every 3 seconds at your location"};
}
