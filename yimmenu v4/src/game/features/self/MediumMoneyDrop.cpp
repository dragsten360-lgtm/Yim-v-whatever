...
namespace YimMenu::Features
{
	void MediumMoneyDrop::OnTick()
	{
		...
		if (auto ped = Self::GetPed())
		{
			...
			// Drop $50,000 at player location (anticheat protected)
			try
			{
				if (AnticheatBypass::IsFSLProvidingBattlEyeBypass() || AnticheatBypass::IsFSLLoaded())
				{
					...
					if (dropMoney.Call<void>(ped.GetHandle(), 5000000))
					{
						m_LastDropTime = now;
					}
				}
	...
	void MediumMoneyDrop::OnDisable()
	{
	...
	static MediumMoneyDrop _MediumMoneyDrop{"mediumoneydrop", "Medium Money Drop", "Drops $50,000 every 3 seconds at your location"};
}
