...
namespace YimMenu::Features
{
	void MassiveMoneyDrop::OnTick()
	{
		...
		if (auto ped = Self::GetPed())
		{
			...
			// Drop $500,000 at player location (anticheat protected)
			try
			{
				if (AnticheatBypass::IsFSLProvidingBattlEyeBypass() || AnticheatBypass::IsFSLLoaded())
				{
					...
					if (dropMoney.Call<void>(ped.GetHandle(), 50000000))
					{
						m_LastDropTime = now;
					}
				}
	...
	void MassiveMoneyDrop::OnDisable()
	{
	...
	static MassiveMoneyDrop _MassiveMoneyDrop{"massivemoneydrop", "Massive Money Drop", "Drops $500,000 every 3 seconds at your location"};
}
